  ...
  return (
    <div ref={rowContainer} className={`w-full flex items-center gap-3 my-12 scroll-smooth ${flag ? 'overflow-x-scroll scrollbar-none':'overflow-x-hidden flex-wrap justify-center' }`}>
        ...
        { data && data.length>0 ? (data.map(item =>(
            <div key={item?.id} className='w-275 h-[225px] min-w-[275px] md:w-300 md:min-w-[300px] my-12 bg-cardOverlay rounded-lg px-4 py-2 backdrop-blur-lg hover:drop-shadow-lg flex flex-col items-center justify-between relative'>
                ...
                <div className='w-full flex flex-col items-end justify-end'>
                    ...
                    <div className='flex items-center gap-8'>
                        <p className='text-lg text-headingColor font-semibold'>
                            <span className='text-lg text-red-500'><BiRupee className='inline'/></span>{item?.price}
                        </p>
                    </div>
                </div>
            </div>
